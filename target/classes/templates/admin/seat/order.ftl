...
<head>
  ...
  <title>${siteName!""}|座位预约记录-${title!""}</title>
                ...
                <div class="table-responsive">
                  <table class="table table-bordered">
                    ...
                    <#if pageBean.content?size gt 0>
                      <#list pageBean.content as seatOrder>
                        <tr>
                          <td style="vertical-align:middle;">
                            <label class="lyear-checkbox checkbox-primary">
                              <input type="checkbox" name="ids[]" value="${seatOrder.id}"
                                      <#if seatOrder.getStatus()==1>
                                       disabled
                                      </#if>
                              >
                              <span></span>
                          ...
                          <td align="center">${seatOrder.getReadingRoom().getName()}</td>
                          <td align="center">${seatOrder.getSeat().getYAxis()}排${seatOrder.getSeat().getXAxis()}座</td>
                          <td align="center">${seatOrder.getSubscribeTime()?string("yyyy-MM-dd")}&nbsp
                            <#if seatOrder.getTimeCode()==1>
                                上午
                              <#else>
                                下午
                            </#if>
  ...
  function deleteReq(id,url){
    $.ajax({
      ...
      success:function(data){
        if(data.code == 0){
          showSuccessMsg('座位订单删除成功!',function(){
